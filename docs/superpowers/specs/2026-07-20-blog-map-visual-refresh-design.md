...
- **Deterministic layout:** replace `Math.random()` jitter in the `posts` construction with the existing seeded LCG pattern (same generator as `hubPos`, different fixed seed), and fix the world size to constants (2400×1600 world units; it was previously derived from the viewport at load, so a different window size produced a different map). Same map every visit, in every viewport — framing auto-fits.
